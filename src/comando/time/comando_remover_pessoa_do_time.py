...
class ComandoRemoverPessoaDoTime(Comando):
    ...
    def executar(self) -> str:
        ...
        self.time = Time(dados_time_list[0], dados_time_list[1], dados_time_list[2], int(dados_time_list[3]))
        dados_pessoa = self.banco.ler_pessoa(self.cpf)
        if dados_pessoa == "Pessoa não encontrada." or dados_pessoa == "Sem pessoas cadastradas.":
            return dados_pessoa
        dados_pessoa_list = dados_pessoa.split(';')
        self.pessoa = Pessoa(dados_pessoa_list[0], dados_pessoa_list[1], dados_pessoa_list[2])
